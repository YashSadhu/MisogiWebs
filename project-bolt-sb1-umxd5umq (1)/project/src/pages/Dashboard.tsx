...
const Dashboard: React.FC = () => {
    ...
    todayMeds.forEach(med => {
      med.timeSlots.forEach(timeSlot => {
        if (isUpcomingTime(timeSlot)) {
          const doseKey = `${med.id}-${timeSlot}-${currentDate}`;
          if (!recentlyTaken.has(doseKey)) {
  ...
  const markDoseTaken = async (medicationId: string, timeSlot: string) => {
    ...
    const doseKey = `${medicationId}-${timeSlot}-${currentDate}`;
    setRecentlyTaken(prev => new Set(prev).add(doseKey));
  ...
  const calculateStreak = () => {
    ...
    while (true) {
      ...
      const allTaken = dayLogs.every(log => 
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card className="bg-gradient-to-br from-teal-500 to-teal-600 text-white transform hover:scale-105 transition-transform">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center text-white">
        ...
        <Card className="bg-gradient-to-br from-amber-500 to-amber-600 text-white transform hover:scale-105 transition-transform">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center text-white">
        ...
        {upcomingDoses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {upcomingDoses.map((dose, index) => (
              <Card key={`upcoming-${index}`} className="transform hover:scale-105 transition-transform">
          ...
          <Card>
            <CardContent className="py-6">
              <div className="flex items-center justify-center text-gray-600">
                ...
                <p>All caught up! No upcoming doses in the next hour</p>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {overdueDoses.map((dose, index) => (
              <Card key={`overdue-${index}`} className="border-l-4 border-red-500 transform hover:scale-105 transition-transform">
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between">
                    <div>
      ...
      {!notificationsEnabled && (
        <Card className="mb-8 bg-gray-50 transform hover:scale-105 transition-transform">
          <CardContent className="py-6">
