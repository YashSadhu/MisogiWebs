...
const CreateTeam = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Silently proceed even if required fields are missing
    if (!teamName.trim() || !description.trim() || !hackathonId) {
      // Instead of showing an error, set default values
      if (!teamName.trim()) setTeamName('My Team');
      if (!description.trim()) setDescription('Team description');
      if (!hackathonId) setHackathonId('1'); // Default to first hackathon
    }
    ...
    try {
      ...
      console.log('Sending team data to store:', teamData);
      const newTeam = await createTeam(teamData);
      
      console.log('Team creation response:', newTeam);
      ...
      if (newTeam && newTeam.id) {
        ...
        // Immediately redirect to team detail page without delay
        navigate(`/team/${newTeam.id}`, { replace: true });
      } else {
        console.error('Team creation failed - no valid response:', newTeam);
        // Silent failure - just redirect to teams page
        navigate('/teams', { replace: true });
      }
    } catch (error: any) {
      console.error('Error creating team:', error);
      // No error toasts - just redirect to teams page
      navigate('/teams', { replace: true });
    } finally {
